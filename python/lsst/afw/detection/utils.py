...
def makeDiaSourceFromSource(source):
    ...
    # Ignore NaNs if SHAPE_UNWEIGHTED_BAD is set
    if repr(source.getIxx()) == "nan" and source.getFlagForDetection() & 0x20:
        diaSource.setIxx(0.0)
        diaSource.setIxxErr(0.0)
        diaSource.setIyy(0.0)
        diaSource.setIyyErr(0.0)
        diaSource.setIxy(0.0)
        diaSource.setIxyErr(0.0)
    else:
