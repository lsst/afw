...
from lsst.afw.geom.skyWcs import makeModifiedWcs
...
def makeDistortedTanWcs(tanWcs, pixelToFocalPlane, focalPlaneToFieldAngle):
    ...
    Parameters
    ----------
    tanWcs : lsst.afw.geom.skyWcs.SkyWcs
    ...
    LookupError
        If 2-dimensional Frames with Domain "PIXEL" and "IWCS"
        are not all found.
    ...
    Our input TAN WCS is:
        tanWcs = PIXEL frame -> pixelToIwc -> IWC frame ->  iwcToSky -> SkyFrame
    See lsst.afw.geom.skyWcs.SkyWcs for a description of these frames.
    tanWcs may also contain an ACTUAL_PIXELS frame before the PIXEL frame;
    if so it will be preserved, but it is irrelevant to the computation
    and so not discussed further.

    Our desired WCS must still contain the PIXEL and IWC frames.
    The distortion will be inserted just after the PIXEL frame,
    So the new WCS will be as follows:
        wcs = PIXEL frame -> pixelToDistortedPixel -> pixelToIwc -> IWC frame -> iwcToSky -> SkyFrame
    ...
    return makeModifiedWcs(pixelTransform=pixelToDistortedPixel, wcs=tanWcs)
...
def computePixelToDistortedPixel(pixelToFocalPlane, focalPlaneToFieldAngle):
    ...
    the initial WCS will contain these frames and transforms:
        PIXEL frame -> pixelToIwc -> IWC frame ->  gridToIwc -> SkyFrame
