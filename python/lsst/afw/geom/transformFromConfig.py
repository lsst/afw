...
__all__ = ["transformRegistry", "OneTransformConfig"]
...
class IdentityTransformConfig(Config):
    nDimensions = Field(
        ...
        default = 2,  # Backward-compatible with IdentityXYTransformConfig
        optional = True,
    )
    ...
    def validate(self):
        ...
        if self.nDimensions <= 0:
            raise RuntimeError(
                "invalid identity transform with %d dimensions" %
                (self.nDimensions,))
...
def identityFactory(config):
    ...
    """
    return makeIdentityTransform(config.nDimensions)
