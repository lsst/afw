...
#-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
#
# Subclass threading to watch for changes to the file "fileOfInterest" (usually ~/.ds9.xy)
#
import stat, threading
            ...
            if self.getMtime() != self.mtime: # file's changed
                ...
                try:
                    ...
                    print >> sys.stderr, "ds9.callbacks[%s](%s, %s, %s) failed: %s" % \
                        (k, k, x, y, e)
#
# Make sure that there's a thread running to look for ds9 events
#
try:
    type(callbackThread)
except NameError:
    import callbackUtils
    from callbackUtils import setCallback # allow user to say ds9.setCallback('u', func)

    callbackUtils.callbacks = {}
    with open(callbackUtils.callbackAnalysisFile, "w") as fd:
        print >> fd, "# Machine generated by lsst/afw/display/ds9.py"
        print >> fd, "# Modifications will be overwritten, so don't bother\n"

        for k in callbackUtils.activeKeys:
            callbackUtils.setCallback(k) # sets callback to a no-op

            print >> fd, """Write the current cursor position to %s
*.fits
bind %s
echo "%s $x(wcsa) $y(wcsa)" > %s""" % (callbackUtils.callbackFile, k, k, callbackUtils.callbackFile)
    #
    # Load the definitions of keys that we'll handle into ds9
    #
    ds9Cmd("analysis load %s" % callbackUtils.callbackAnalysisFile)
    #
    # Now start the thread that watches callbackFile and dispatches the callback functions
    #
    callbackThread = callbackUtils.Ds9CallbackThread(callbackUtils.callbackFile, verbose=False)
