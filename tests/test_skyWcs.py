...
from lsst.afw.geom.skyWcs import SkyWcs, makeCdMatrix, makeWcsPairTransform, \
    getIntermediateWorldCoordsToSky, getPixelToIntermediateWorldCoords, \
    makeFlippedWcs, makeModifiedWcs, makeTanSipWcs
from lsst.afw.geom.detail import getCdMatrixFromMetadata, getSipMatrixFromMetadata, makeSimpleWcsMetadata
...
class SkyWcsBaseTestCase(lsst.utils.tests.TestCase):
    ...
    def assertSkyWcsAstropyWcsAlmostEqual(self, skyWcs, astropyWcs, bbox,
                                          pixAtol=1e-4, skyAtol=1e-4*arcseconds):
        ...
        pixPosArr = np.array([(x, y) for x, y in itertools.product(xArr, yArr)], dtype=float)

        # pixelToSky
        pixPosList = [Point2D(*xy) for xy in pixPosArr]
        skyPosList = skyWcs.pixelToSky(pixPosList)
        skyDegArr = np.array([list(skyPos.getPosition(degrees)) for skyPos in skyPosList])
        astropySkyDegArr = astropyWcs.all_pix2world(pixPosArr, 0)
        astropySkyPosList = [IcrsCoord(p[0]*degrees, p[1]*degrees) for p in astropySkyDegArr]
        self.assertCoordListsAlmostEqual(skyPosList, astropySkyPosList, maxDiff=skyAtol)

        # astropy round trip
        astropyPixPosRoundTrip = astropyWcs.all_world2pix(astropySkyDegArr, 0)
        assert_allclose(pixPosArr, astropyPixPosRoundTrip, atol=pixAtol)

        # SkyWcs round trip
        pixPosListRoundTrip = skyWcs.skyToPixel(skyPosList)
        pixPosArrRoundTrip = np.array([list(pixPos) for pixPos in pixPosListRoundTrip])
        assert_allclose(pixPosArr, pixPosArrRoundTrip, atol=pixAtol)

        # skyToPixel astropy vs SkyWcs
        astropySkyToPixArr = astropyWcs.all_world2pix(skyDegArr, 0)
        assert_allclose(pixPosArrRoundTrip, astropySkyToPixArr, atol=pixAtol)


class SimpleSkyWcsTestCase(SkyWcsBaseTestCase):
    """Test simple WCS constructor of SkyWcs
    """
    ...
    def checkTanWcs(self, crval, orientation, flipX):
        ...
        # the pixel origin is cached, so call this twice
        for i in range(2):
        ...
        # the result of getPixelScale() is cached, so call this twice
        for i in range(2):
        ...
        shiftedWcsCopy = SkyWcs(shiftedMetadata)
    ...
    def testMakeModifiedWcs(self):
        ...
        wcs = SkyWcs(crpix=self.crpix, crval=self.crvalList[0], cdMatrix=cdMatrix)
        pixelTransform = makeRadialTransform([0.0, 1.0, 0.0, 0.0011])  # arbitrary but reasonable
        modifiedWcs = makeModifiedWcs(pixelTransform, wcs)
    ...
    def testAgainstAstropyWcs(self):
        ...
        for crval, orientation, flipX, projection in itertools.product(self.crvalList,
            ...
            skyWcs = SkyWcs(crpix=self.crpix, crval=crval, cdMatrix=cdMatrix, projection=projection)
...
class MetadataWcsTestCase(SkyWcsBaseTestCase):
    ...
    def testBasics(self):
        ...
        SkyWcs(self.metadata, strip = True)
...
class TestTanSipTestCase(SkyWcsBaseTestCase):
    ...
    def testMakeTanSipWcs(self):
        referenceWcs = SkyWcs(self.metadata, strip=False)
