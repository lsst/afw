...
class FootprintSetTestCase(unittest.TestCase):
    ...
    def testFootprintControl(self):
        ...
        self.assertFalse(fctrl.isCircular()[0])
        self.assertFalse(fctrl.isIsotropic()[0])
        self.assertFalse(fctrl.isCircular()[1])
        self.assertFalse(fctrl.isIsotropic()[1])

        #-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
        fctrl = afwDetect.FootprintControl()
        fctrl.growLeft(False)
        self.assertFalse(fctrl.isLeft()[0]) # it's now set
        self.assertFalse(fctrl.isLeft()[1]) # ... but False

        #-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
        fctrl = afwDetect.FootprintControl(True, False, False, False)
        self.assertFalse(fctrl.isLeft()[0])
        self.assertFalse(fctrl.isRight()[0])
        self.assertFalse(fctrl.isUp()[0])
        self.assertFalse(fctrl.isDown()[0])

        self.assertFalse(fctrl.isLeft()[1])
