...
class ImageFitsTestCase(TestCase):
    ...
    def check_roundtrip_image_invariants(
        ...
        *,
        ...
        rtol : `float`, optional
            ...
            reader and astropy, to account for slight differences in order of
            operations when undoing quantization.  Defaults to zero.
        atol : `float`, optional
    ...
    def check_quantized_roundtrip(
        ...
        *,
        ...
        astropy_rtol: float = 0.0,
        astropy_atol: float = 0.0,
        safe_dtypes: Sequence[np.typing.DTypeLike] = (),
    ) -> None:
        ...
        roundtrip_atol : `float`
            ...
            image.
        astropy_rtol : `float`, optional
            Relative tolerance for floating-point comparisons between our
            reader and astropy, to account for slight differences in order of
            operations when undoing quantization.  Defaults to zero.
        astropy_atol : `float`, optional
            Absolute tolerance for floating-point comparisons between our
            reader and astropy, to account for slight differences in order of
            operations when undoing quantization.  Defaults to zero.
        safe_dtypes : `~collections.abc.Sequence` [`numpy.dtype`], optional
        ...
        with self.check_roundtrip_image_invariants(
            masked_image.image, compression, safe_dtypes, atol=astropy_atol, rtol=astropy_rtol
        ) as image_rt:
            self.assertImagesAlmostEqual(image_rt, masked_image.image, atol=roundtrip_atol)
        with self.check_roundtrip_masked_image_invariants(
            masked_image, compression, safe_dtypes=safe_dtypes, atol=astropy_atol, rtol=astropy_rtol
        ) as masked_image_rt:
