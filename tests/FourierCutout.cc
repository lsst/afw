...
BOOST_AUTO_TEST_CASE(CutoutOperatorTest) {
    ...
    FourierCutout::RealT val = 2.0;
    //test scalar operators
    BOOST_CHECK_NO_THROW(a = val);
    BOOST_CHECK_EQUAL(a(0,0), val);
    BOOST_CHECK_NO_THROW(a -= val);
    BOOST_CHECK_EQUAL(a(1,1), static_cast<FourierCutout::RealT>(0));
    BOOST_CHECK_NO_THROW(a += val);
    ...
    BOOST_CHECK_NO_THROW(b -= a);
    BOOST_CHECK_EQUAL(b(3,2), static_cast<FourierCutout::RealT>(0));
    BOOST_CHECK_NO_THROW(b += a);
...
BOOST_AUTO_TEST_CASE(CutoutStackTest) {
    ...
    for(int i = 0; i < depth; ++i, data += cutoutSize) {
        ...
        *cutoutPtr = static_cast<FourierCutout::RealT>(i);
    ...
    for(int i = 0; i < vectorLength; ++i) {
        ...
        for( ; j != end; ++j) {
            BOOST_CHECK_EQUAL((*j), static_cast<FourierCutout::RealT>(i + begin));            
        }    
    }
