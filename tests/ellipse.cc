...
inline bool approx(double a, double b, double tol = 1E-8) { return std::fabs(a - b) <= tol; }
...
Eigen::Matrix<double, Function::M, Function::N> computeJacobian(
        Function f, Eigen::Matrix<double, Function::N, 1> const& initial) {
...
struct TransformerTest {
    ...
    static void apply(Core const& core) {
        ...
        BOOST_CHECK_MESSAGE(
                ...
                boost::str(boost::format("Transformer::d failed for %s:\nAnalytic:\n%s\nNumeric:\n%s\n") %
        ...
        BOOST_CHECK_MESSAGE(
                ...
                boost::str(boost::format(
                                   "Transformer::dTransform failed for %s:\nAnalytic:\n%s\nNumeric:\n%s\n") %
...
struct GridTransformTest {
    ...
    static void apply(T const& core) {
        ...
        BOOST_CHECK_MESSAGE(
                ...
                boost::str(
                        boost::format(
                                "GridTransform::getMatrix incorrect %s:\ngetMatrix:\n%s\nTransform:\n%s\n") %
        ...
        Ellipse::GridTransform::DerivativeMatrix d_numeric = computeJacobian(f, input.getParameterVector());
        BOOST_CHECK_MESSAGE(
...
struct ConvolutionTest {
    ...
    static void apply(T core) {
        ...
        BaseCore::Convolution::DerivativeMatrix d_numeric = computeJacobian(f, input.getParameterVector());
        BOOST_CHECK_MESSAGE(
                d_analytic.isApprox(d_numeric, 1E-4),
                boost::str(boost::format("Convolve::d failed for %s:\nAnalytic:\n%s\nNumeric:\n%s\n") %
                           core.getName() % d_analytic % d_numeric));
    }
};
}
...
BOOST_AUTO_TEST_CASE(Transformer) { afwEllipses::invokeCoreTest<afwEllipses::TransformerTest>(false); }

BOOST_AUTO_TEST_CASE(GridTransform) { afwEllipses::invokeCoreTest<afwEllipses::GridTransformTest>(false); }

BOOST_AUTO_TEST_CASE(Convolution) { afwEllipses::invokeCoreTest<afwEllipses::ConvolutionTest>(false); }
