...
void test1()
{
	...
	else
	{
		...
		if (fits_create_file(&ff_out, string("!" + gFilename + "_cfitsioInOut.fit").c_str(), &status) != 0)
		{
...
void test2()
{
	...
	image->writeFits(string("!" + gFilename + "_imageInOut.fit").c_str());
}
...
void test4()
{
	...
	cfitsio::fitsfile *ff_out = 0;
	if (fits_create_file(&ff_out, string("!" + gFilename + "_fitsOut.fit").c_str(), &status) != 0)
	{
...
void test6()
{
	...
	char *ramFile = NULL;
	size_t ramFileLen = 0;
	image->writeFits(string(gFilename + "_imageOut.fit").c_str());
	image->writeFits(&ramFile, &ramFileLen);
	
	//Write the RAM FITS file to disk
	ofstream ofs;
	ofs.open(string(gFilename + "_imageRamOut.fit").c_str());
	if (ofs)
		ofs.write(ramFile, ramFileLen);
	ofs.close();
}
...
void test7()
{
	...
	//Write the RAM FITS file to disk
	ofstream ofs;
	ofs.open(string(gFilename + "_exposureRamOut.fit").c_str());
	if (ofs)
		ofs.write(ramFile, ramFileLen);
	ofs.close();
}
