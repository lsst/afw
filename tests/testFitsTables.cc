...
struct EqualityCompare {
    bool operator()(lsst::afw::geom::Span const& a, lsst::afw::geom::Span const& b) const {
    ...
    bool operator()(lsst::afw::detection::PeakRecord const& a,
                    lsst::afw::detection::PeakRecord const& b) const {
        return (*this)(a.getFx(), b.getFx()) && (*this)(a.getFy(), b.getFy()) &&
...
BOOST_AUTO_TEST_CASE(testFits) {
    ...
    Key<float> a_c_f =
            schema.addField<float>("a_c_f",
                                   "an extremely long string for documenting this float field that will "
                                   "require use of the FITS long-string convention that splits long values "
                                   "up and puts them on different keys using CONTINUE.",
                                   "barn");
    ...
    readVector.getSchema().forEach(func2);
