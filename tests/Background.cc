...
BOOST_AUTO_TEST_CASE(BackgroundTestImages) {

    {
        ...
        for (vector<string>::iterator imgfile = imgfiles.begin(); imgfile != imgfiles.end(); ++imgfile) {
            ...
            float const req_mean =
                static_cast<float>( boost::any_cast<double>(fitsHdr->findUnique("MEANREQ", true)->getValue()) );
            float const req_stdev =
                static_cast<float>( boost::any_cast<double>(fitsHdr->findUnique("SIGREQ", true)->getValue()) );
