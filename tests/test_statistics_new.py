...
#        self.assertRaises(lsst.pex.exceptions.InvalidParameterError, tst)
    ...
    def testStats1(self):
        stats = afwMath.standardStatistics(self.image, count=True, stddev=True, mean=True, sum=True)

        self.assertEqual(stats["count"],
                         self.image.getWidth()*self.image.getHeight())
        self.assertEqual(stats["count"]*stats["mean"],
                         stats["sum"])
        self.assertEqual(stats["mean"], self.val)
...
#        self.assertEqual(stats.getValue(afwMath.STDEV), 0)
    ...
    def testStats2(self):
        stats = afwMath.standardStatistics(
            self.image, stddev=True, mean=True, errors=True)

        self.assertEqual(stats["mean"], self.image.get(0, 0))
        self.assertEqual(
            stats["meanErr"],
            stats["stddev"]/math.sqrt(self.image.getWidth()*self.image.getHeight()))

    def testStats3(self):
        stats = afwMath.standardStatistics(self.image, count=True)

        def getMean():
            stats["mean"]

        self.assertRaises(KeyError, getMean)
    ...
    def testStatsZebra(self):
        ...
        stats = afwMath.standardStatistics(image2,
                                       count=True, stddev=True, mean=True, errors=True)
        mean = stats["mean"]
        n = stats["count"]
        sd = stats["stddev"]

        self.assertEqual(mean, image2.get(0, 0) + 0.5)
        self.assertEqual(sd, 1/math.sqrt(4.0)*math.sqrt(n/(n - 1)))
#        self.assertAlmostEqual(
#            stats["meanErr"], sd/math.sqrt(image2.getWidth()*image2.getHeight()), 10)

        meanSquared = afwMath.standardStatistics(
            image2, meanSquared=True)["meanSquared"]
        self.assertEqual(meanSquared, 0.5*(image2.get(0, 0) ** 2 +
                                          image2.get(0, 1) ** 2))
    ...
    def testStatsStdevclip(self):
        ...
        stats = afwMath.standardStatistics(
            image2, clippedStddev=True, count=True, sum=True)
        self.assertEqual(stats["clippedStddev"], 0)
        #
        # Check we get the correct sum even when clipping
        #
        self.assertEqual(
            stats["count"] * afwMath.standardStatistics(
                image2, mean=True)["mean"],
            stats["sum"])

    def testMedian(self):
        """Test the median code"""
        stats = afwMath.standardStatistics(self.image, median=True)

        self.assertEqual(stats["median"], self.val)

#        values = [1.0, 2.0, 3.0, 2.0]
#        self.assertEqual(
#            afwMath.standardStatistics(values, stats["median"])["median"],
#            2.0)

    def testIqrange(self):
        """Test the inter-quartile range"""
        stats = afwMath.standardStatistics(self.image, iqrange=True)
        self.assertEqual(stats["iqrange"], 0)

    def testMeanClip(self):
        """Test the 3-sigma clipped mean"""
        stats = afwMath.standardStatistics(self.image, clippedMean=True)
        self.assertEqual(stats["clippedMean"], self.val)

    def testStdevClip(self):
        """Test the 3-sigma clipped standard deviation"""
        stats = afwMath.standardStatistics(self.image, clippedStddev=True)
        self.assertEqual(stats["clippedStddev"], 0)

    def testVarianceClip(self):
        """Test the 3-sigma clipped variance"""
        stats = afwMath.standardStatistics(self.image, clippedVariance=True)
        self.assertEqual(stats["clippedVariance"], 0)
...
#        for useImage in [True, False]:
        for useImage in [True, ]:
            ...
            self.assertEqual(
                afwMath.standardStatistics(self.image, max=True)["max"],
                self.val)
            self.assertEqual(
                afwMath.standardStatistics(self.image, mean=True)["mean"],
                self.val)

            sctrl = afwMath.NewStatisticsControl()

            sctrl.isNanSafe = False
            self.assertFalse(np.isfinite(afwMath.standardStatistics(
                self.image, mean=True, sctrl=sctrl)["mean"]))
    ...
    def testSampleImageStats(self):
        ...
        for imgfile in imgfiles:
            ...
            img = dimg.getImage()
            stats = afwMath.standardStatistics(img, mean=True, stddev=True)

            # print trueMean, mean, trueStdev, stdev
#            self.assertAlmostEqual(stats["mean"], trueMean, 8)
#            self.assertAlmostEqual(stats["stddev"], trueStdev, 8)
    ...
    def testStatisticsRamp(self):
        ...
        stats = afwMath.standardStatistics(
            img, count=True, stddev=True, mean=True)
        testmean = stats["mean"]
        teststdev = stats["stddev"]

        self.assertEqual(stats["count"], nx*ny)
        self.assertEqual(testmean, mean)
        self.assertAlmostEqual(teststdev, stdev)

        stats = afwMath.standardStatistics(
            img, stddev=True, mean=True, errors=True)
        mean = stats["mean"]
        meanErr = stats["meanErr"]
        sd = stats["stddev"]
        ...
        stats = afwMath.standardStatistics(img, median=True)
        self.assertEqual(z0 + dzdx*(nx - 1)/2.0,
                         stats["median"])

        stats = afwMath.standardStatistics(img, iqrange=True)
        self.assertEqual(dzdx*(nx - 1)/2.0, stats["iqrange"])

        stats = afwMath.standardStatistics(img, clippedMean=True)
        self.assertEqual(z0 + dzdx*(nx - 1)/2.0,
                         stats["clippedMean"])
...
#        values = [1.0, 2.0, 3.0, 2.0]
#        self.assertEqual(afwMath.makeStatistics(
#            values, afwMath.MEDIAN).getValue(), 2)
#        self.assertEqual(afwMath.makeStatistics(
#            sorted(values), afwMath.MEDIAN).getValue(), 2)
#
...
#        self.assertAlmostEqual(
#            weighted.getValue(afwMath.MEAN), mean)
#        self.assertAlmostEqual(weighted.getError(afwMath.MEAN)**2,
#                               variance/npix)
#        self.assertAlmostEqual(weighted.getError(afwMath.MEANCLIP)**2,
#                               variance/npix)
...
#        img.set(0)
#        stats = afwMath.standardStatistics(img, clippedMean=True)
#        self.assertEqual(stats["clippedMean"], 0)
