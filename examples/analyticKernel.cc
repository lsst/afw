...
int main() {
    ...
    for (unsigned int y = 0; y < 2; ++y) {
        for (unsigned int x = 0; x < 2; ++x) {
            gaussSpVarKernel.computeKernelParametersFromSpatialModel(kernelParams, static_cast<double>(x),
                                                                     static_cast<double>(y));
            cout << boost::format("GaussianKernel at x=%d, y=%d; xSigma = %7.2f, ySigma=%7.2f:\n\n") % x % y %
                            kernelParams[0] % kernelParams[1];

            lsst::afw::math::printKernel(gaussSpVarKernel, true, static_cast<double>(x),
                                         static_cast<double>(y));
        }
    }
}
