...
public:
    ...
    IntegrandT operator()(IntegrandT const r, IntegrandT const) const { return (_k - _kr * r * r) * r; }

    // for this example we have an analytic answer to check
    IntegrandT getAnalyticVolume(IntegrandT const r1, IntegrandT const r2, IntegrandT const theta1,
                                 IntegrandT const theta2) {
        return ((theta2 - theta1) * ((0.5 * _k * r2 * r2 - (1.0 / 3.0) * _kr * r2 * r2 * r2) -
...
int main() {
    ...
    std::cout << "2D integrate: functor = " << parab_volume_integrate
              << "  function = " << parab_volume_integrate_func << "  analytic = " << parab_volume_analytic
              << std::endl;
