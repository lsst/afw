...
void afwForm::KernelFormatter::write(
    ...
    LOGL_TRACE9(_log, "KernelFormatter write start");
...
dafBase::Persistable* afwForm::KernelFormatter::read(
    ...
    LOGL_TRACE9(_log, "KernelFormatter read start");
    afwMath::Kernel* kp;
    if (typeid(*storage) == typeid(dafPersist::BoostStorage)) {
        LOGL_TRACE9(_log, "KernelFormatter read BoostStorage");
        dafPersist::BoostStorage* boost =
            dynamic_cast<dafPersist::BoostStorage*>(storage.get());
        boost->getIArchive() & kp;
        LOGL_TRACE9(_log, "KernelFormatter read end");
        return kp;
    }
    else if (typeid(*storage) == typeid(dafPersist::XmlStorage)) {
        LOGL_TRACE9(_log, "KernelFormatter read XmlStorage");
        dafPersist::XmlStorage* xml =
            dynamic_cast<dafPersist::XmlStorage*>(storage.get());
...
void afwForm::KernelFormatter::delegateSerialize(
    ...
    LOGL_TRACE9(_log, "KernelFormatter delegateSerialize start");
    afwMath::Kernel* kp =
    ...
    LOGL_TRACE9(_log, "KernelFormatter delegateSerialize end");
}
