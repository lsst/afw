...
Camera::Builder Camera::rebuild() const { return Camera::Builder(*this); }
...
private:
    PersistenceHelper()
            : schema(),
              ...
              pupilFactoryName(schema.addField<std::string>(
                      "pupilFactoryName", "Fully-qualified name of a Python PupilFactory class", "", 0)),
...
std::shared_ptr<Camera const> Camera::Builder::finish() const {
    ...
    for (auto const &pair : getIdMap()) {
        ...
        std::vector<TransformMap::Connection> detectorConnections =
                getDetectorBuilderConnections(detectorBuilder);
        ...
        if ((std::find(allDetConnections.begin(), allDetConnections.end(), connection) ==
             allDetConnections.end()) &&
            (std::find(allDetConnections.begin(), allDetConnections.end(), invConnection) ==
             allDetConnections.end())) {
            connections.push_back(TransformMap::Connection{
                    detectorBuilder.getOrientation().makeFpPixelTransform(detectorBuilder.getPixelSize()),
                    getNativeCameraSys(), detectorBuilder.getNativeCoordSys()});
        }
        connections.insert(connections.end(), getDetectorBuilderConnections(detectorBuilder).begin(),
    ...
    return std::shared_ptr<Camera>(
            new Camera(_name, std::move(detectors), std::move(transformMap), _pupilFactoryName));
}
...
Iter findConnection(Iter first, Iter last, CameraSys const &toSys) {
    return std::find_if(first, last, [&toSys](auto const &connection) { return connection.toSys == toSys; });
}

}  // namespace

void Camera::Builder::setTransformFromFocalPlaneTo(
        CameraSys const &toSys, std::shared_ptr<afw::geom::TransformPoint2ToPoint2 const> transform) {
    if (toSys.hasDetectorName()) {
        throw LSST_EXCEPT(pex::exceptions::LogicError,
                          (boost::format("%s should be added to Detector %s, not Camera") %
                           toSys.getSysName() % toSys.getDetectorName())
                                  .str());
    }
    auto iter = findConnection(_connections.begin(), _connections.end(), toSys);
    if (iter == _connections.end()) {
        _connections.push_back(TransformMap::Connection{transform, getNativeCameraSys(), toSys});
    } else {
