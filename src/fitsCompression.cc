...
ImageCompressionOptions::ImageCompressionOptions(
    ImageCompressionOptions::CompressionAlgorithm algorithm_,
    int rows,
...
ImageScalingOptions::ImageScalingOptions(
    ...
    double bscale_,
    double bzero_
) : algorithm(algorithm_), bitpix(bitpix_), fuzz(fuzz_),
    seed(std::abs(seed_ - 1) % (N_RANDOM - 1) + 1),  // zero is bad (cfitsio uses non-deterministic method)
    ...
    if (numUnique < range) {
        ...
        switch (algorithm) {
          ...
          case ImageScalingOptions::STDEV_BOTH:
            // Put median right in the middle: images with an equal abundance of positive and negative values
            imageVal = median;
            diskVal = cfitsioPadding ? 0.5*N_RESERVED_VALUES : 0;
            break;
          default:
