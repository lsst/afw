...
std::string tail(std::string const & s, std::size_t n) {
        ...
        for (auto iter = _impl->asList().begin(); iter != _impl->asList().end(); ++iter) {
            ...
            if (flagPos != std::string::npos) {
                ...
                if (endswith(iter->ttype, "_err")) {
                    aliases.set(tail(iter->ttype, 4) + "_fluxSigma", iter->ttype);
                } else {
                ...
                if (iter->tccls == "Covariance(Point)") {
                    aliases.set(tail(iter->ttype, 4) + "_xSigma", iter->ttype + "_xSigma");
                    aliases.set(tail(iter->ttype, 4) + "_ySigma", iter->ttype + "_ySigma");
                    aliases.set(tail(iter->ttype, 4) + "_x_y_Cov", iter->ttype + "_x_y_Cov");
                } else if (iter->tccls == "Covariance(Moments)") {
                    aliases.set(tail(iter->ttype, 4) + "_xxSigma", iter->ttype + "_xxSigma");
                    aliases.set(tail(iter->ttype, 4) + "_yySigma", iter->ttype + "_yySigma");
                    aliases.set(tail(iter->ttype, 4) + "_xySigma", iter->ttype + "_xySigma");
                    aliases.set(tail(iter->ttype, 4) + "_xx_yy_Cov", iter->ttype + "_xx_yy_Cov");
                    aliases.set(tail(iter->ttype, 4) + "_xx_xy_Cov", iter->ttype + "_xx_xy_Cov");
                    aliases.set(tail(iter->ttype, 4) + "_yy_xy_Cov", iter->ttype + "_yy_xy_Cov");
                }
            }
        }
    }
