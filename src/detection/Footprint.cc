...
PTR(Footprint) growFootprintImpl(
    ...
    // spans which will be added to the footprint.
    for (auto spanIter = foot.getSpans().begin(); spanIter != foot.getSpans().end(); ++spanIter) {
        for (auto it = element.begin(); it != element.end(); ++it) {
            ...
            int yval = (*spanIter)->getY() + it->getY();
            ...
            for (auto span = spans[yval].cbegin(); span != spans[yval].cend(); ++span) {
                int start = span->first, end = span->second;
                // NB check for end+1 because end value is inclusive. That is,
                // if one span terminates at x=N and another begins at x=N+1,
    ...
    // Now append the spans to the output footprint.
    for (auto y = spans.cbegin(); y != spans.cend(); ++y) {
        int yval = y->first;
        for (auto x = y->second.cbegin(); x != y->second.cend(); ++x) {
            grown->addSpanInSeries(yval, x->first, x->second);
        }
    }
