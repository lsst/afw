...
boost::tuple<double, double, double, double> math::Statistics<Image>::_getStandard(Image const &img, int const flags) {
    ...
    if (n == 0) { throw lsst::pex::exceptions::InvalidParameter("Image has no valid pixels; mean is undefined."); }
    double mean = crude_mean + sum/n;
    if (n == 1) { throw lsst::pex::exceptions::InvalidParameter("Image has only one pixel; stdev is undefined"); }
