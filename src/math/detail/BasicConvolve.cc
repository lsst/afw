        ...
        afwMath::ConvolutionControl const& convolutionControl)  ///< convolution control parameters
{
    ...
    if (kernel.isSpatiallyVarying() && (convolutionControl.getMaxInterpolationDistance() > 1)) {
        ...
        LOGF_TRACE3("lsst.afw.math.convolve", "generic basicConvolve: using linear interpolation");
        mathDetail::convolveWithInterpolation(convolvedImage, inImage, kernel, convolutionControl);

    } else {
        // use brute force
        LOGF_TRACE3("lsst.afw.math.convolve", "generic basicConvolve: using brute force");
        ...
        afwMath::ConvolutionControl const &convolutionControl)       ///< convolution control parameters
{
    ...
    LOGF_TRACE3("lsst.afw.math.convolve", "DeltaFunctionKernel basicConvolve");
        ...
        afwMath::ConvolutionControl const & convolutionControl) ///< convolution control parameters
{
    ...
    if (kernel.isSpatiallyVarying()) {
        LOGF_TRACE5("lsst.afw.math.convolve",
