...
namespace {
    /**
     * @brief Private low-level convolution function that does not set edge pixels.
     *
        ...
        if (kernel.isSpatiallyVarying()) {
            ...
            for (int cnvY = cnvStartY; cnvY != cnvEndY; ++cnvY) {
                ...
                for (int cnvX = cnvStartX; cnvX != cnvEndX; ++cnvX, ++inImLoc.x(), ++cnvImIter) {
                    ...
                    *cnvImIter = afwMath::convolveAtAPoint<OutImageT, InImageT>(inImLoc, kernelLoc, kWidth, kHeight);
                    if (doNormalize) {
        ...
        if (kernel.isSpatiallyVarying()) {
            ...
            for (int cnvY = cnvStartY; cnvY != cnvEndY; ++cnvY) {
                ...
                for (int cnvX = cnvStartX; cnvX != cnvEndX; ++cnvX, ++inImLoc.x(), ++cnvImIter) {
                    ...
                    *cnvImIter = afwMath::convolveAtAPoint<OutImageT, InImageT>(inImLoc, kXVec, kYVec);
                    if (doNormalize) {
                        *cnvImIter = *cnvImIter/kSum;
                    }
                }
 ...
 */
template <typename OutImageT, typename InImageT>
inline typename OutImageT::SinglePixel afwMath::convolveAtAPoint(
    typename InImageT::const_xy_locator& imageLocator,
                                        ///< locator for image pixel that overlaps (0,0) pixel of kernel(!)
    afwImage::Image<afwMath::Kernel::PixelT>::const_xy_locator &kernelLocator,
    ...
    for (int y = 0; y != kHeight; ++y) {
        for (int x = 0; x != kWidth; ++x, ++imageLocator.x(), ++kernelLocator.x()) {
            typename afwMath::Kernel::Pixel const kVal = kernelLocator[0];
        ...
        imageLocator  += afwImage::detail::difference_type(-kWidth, 1);
        kernelLocator += afwImage::detail::difference_type(-kWidth, 1);
    }

    imageLocator  += afwImage::detail::difference_type(0, -kHeight);
    kernelLocator += afwImage::detail::difference_type(0, -kHeight);
...
inline typename OutImageT::SinglePixel afwMath::convolveAtAPoint(
    typename InImageT::const_xy_locator& imageLocator,
                                        ///< locator for image pixel that overlaps (0,0) pixel of kernel(!)
    std::vector<afwMath::Kernel::PixelT> const &kernelXList,  ///< kernel column vector
    std::vector<afwMath::Kernel::PixelT> const &kernelYList   ///< kernel row vector
) {
    typedef typename std::vector<afwMath::Kernel::PixelT>::const_iterator k_iter;

    std::vector<afwMath::Kernel::PixelT>::const_iterator kernelYIter = kernelYList.begin();
    ...
    for (k_iter kernelYIter = kernelYList.begin(), end = kernelYList.end();
        ...
        imageLocator += afwImage::detail::difference_type(-kernelXList.size(), 1);
    }
    
    imageLocator += afwImage::detail::difference_type(0, -kernelYList.size());
...
void afwMath::convolve(
    ...
    basicConvolve(convolvedImage, inImage, kernel, doNormalize);
...
void afwMath::convolveLinear(
    ...
    typedef typename OutImageT::x_iterator cnvX_iterator;
    typedef afwMath::LinearCombinationKernel::KernelList kernelListType;
    ...
    std::vector<BasisImagePtr> basisImagePtrList;
    {
        ...
        for (typename kernelListType::const_iterator basisKernelIter = basisKernelList.begin();
            ...
            basicConvolve(*basisImagePtr, inImage, **basisKernelIter, false);
    ...
    for (int cnvY = cnvStartY; cnvY < cnvEndY; ++cnvY) {
        ...
        for (int cnvX = cnvStartX; cnvX != cnvEndX; ++cnvX, ++cnvXIter) {
            ...
            for (unsigned int i = 0; i != basisIterList.size(); ++i) {
                cnvImagePix = afwImage::pixel::plus(cnvImagePix, *basisIterList[i]*kernelCoeffList[i], 1.0);
                ++basisIterList[i];
            }
...
#define convolutionFuncsByType(IMAGE, PIXTYPE1, PIXTYPE2) \
    template IMAGE(PIXTYPE1)::SinglePixel \
        afwMath::convolveAtAPoint<IMAGE(PIXTYPE1), IMAGE(PIXTYPE2)>(IMAGE(PIXTYPE2)::const_xy_locator &, \
            afwImage::Image<afwMath::Kernel::PixelT>::const_xy_locator &, int, int); NL \
    template IMAGE(PIXTYPE1)::SinglePixel \
        afwMath::convolveAtAPoint<IMAGE(PIXTYPE1), IMAGE(PIXTYPE2)>(IMAGE(PIXTYPE2)::const_xy_locator &, \
            std::vector<afwMath::Kernel::PixelT> const &, std::vector<afwMath::Kernel::PixelT> const &); NL \
    template void afwMath::convolve(IMAGE(PIXTYPE1)&, IMAGE(PIXTYPE2) const&, AnalyticKernel const&, bool, int); NL \
    template void afwMath::convolve(IMAGE(PIXTYPE1)&, IMAGE(PIXTYPE2) const&, DeltaFunctionKernel const&, bool, int); NL \
    template void afwMath::convolve(IMAGE(PIXTYPE1)&, IMAGE(PIXTYPE2) const&, FixedKernel const&, bool, int); NL \
    template void afwMath::convolve(IMAGE(PIXTYPE1)&, IMAGE(PIXTYPE2) const&, LinearCombinationKernel const&, bool, int); NL \
    template void afwMath::convolveLinear(IMAGE(PIXTYPE1)&, IMAGE(PIXTYPE2) const&, LinearCombinationKernel const&, int); NL \
    template void afwMath::convolve(IMAGE(PIXTYPE1)&, IMAGE(PIXTYPE2) const&, SeparableKernel const&, bool, int);
