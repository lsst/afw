...
Kernel::PersistenceHelper::PersistenceHelper(int nSpatialFunctions)
        : schema(),
          dimensions(afw::table::PointKey<int>::addFields(schema, "dimensions",
                                                          "dimensions of a Kernel's images", "pixel")),
          center(afw::table::PointKey<int>::addFields(schema, "center", "center point in a Kernel image",
                                                      "pixel")) {
...
Kernel::PersistenceHelper::PersistenceHelper(afw::table::Schema const& schema_)
        : schema(schema_), dimensions(schema["dimensions"]), center(schema["center"]) {
    try {
...
void Kernel::PersistenceHelper::writeSpatialFunctions(
        afw::table::io::OutputArchiveHandle& handle, afw::table::BaseRecord& record,
        std::vector<std::shared_ptr<Kernel::SpatialFunction>> const& spatialFunctionList) const {
...
std::vector<std::shared_ptr<Kernel::SpatialFunction>> Kernel::PersistenceHelper::readSpatialFunctions(
        afw::table::io::InputArchive const& archive, afw::table::BaseRecord const& record) const {
