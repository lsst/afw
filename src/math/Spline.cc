 ...
 */
void
Spline::interpolate(std::vector<double> const& x, ///< points to interpolate at
 ...
 */
void
Spline::derivative(std::vector<double> const& x, ///< points to evaluate derivative at
 ...
 */
                TautSpline::TautSpline(std::vector<double> const& x, ///< points where function's specified
 ...
 */
                void
                TautSpline::calculateTautSpline(std::vector<double> const& x, ///< points where function's specified
                                                std::vector<double> const& y, ///< values of function at tau[]
                                                double const gamma0           ///< control extra knots
                                               )
                {
...
do_cubic(double a, double b, double c, double d, std::vector<double> & roots)
{
    ...
    if(::fabs(r) < sq3) {                   // three real roots
        ...
        roots.push_back(-2*sq*cos((theta + 2*M_PI)/3) - b/3);
        roots.push_back(-2*sq*cos((theta - 2*M_PI)/3) - b/3);
        /*
         * sort roots
         */
 ...
 */
std::vector<double>
Spline::roots(double const value,       ///< desired value
