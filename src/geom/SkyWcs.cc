...
std::shared_ptr<TransformPoint2ToPoint2> makeWcsPairTransform(SkyWcs const& src, SkyWcs const& dst) {
    ...
    return std::make_shared<TransformPoint2ToPoint2>(*transform);
}

SkyWcs::SkyWcs(Point2D const& crpix, coord::IcrsCoord const& crval, Eigen::Matrix2d const& cdMatrix,
               std::string const& projection)
        : SkyWcs(*detail::makeSimpleWcsMetadata(crpix, crval, cdMatrix, projection)) {}
...
Angle SkyWcs::getPixelScale(Point2D const& pixel) const {
    ...
    pixVec.push_back(pixel);
    pixVec.push_back(pixel + Extent2D(side, 0));
    pixVec.push_back(pixel + Extent2D(0, side));
    ...
    double skyAreaSq = square(skyDx[0] * skyDy[1] - skyDx[1] * skyDy[0]) +
                       square(skyDx[1] * skyDy[2] - skyDx[2] * skyDy[1]) +
                       square(skyDx[2] * skyDy[0] - skyDx[0] * skyDy[2]);
    return (std::pow(skyAreaSq, 0.25) / side) * radians;
}

Angle SkyWcs::getPixelScale() const {
    if (_pixelScaleAtOrigin.asRadians() == 0) {
        _pixelScaleAtOrigin = getPixelScale(getPixelOrigin());
    }
    return _pixelScaleAtOrigin;
}

Point2D SkyWcs::getPixelOrigin() const {
    if (!_pixelOrigin) {
        _pixelOrigin = std::make_shared<Point2D>(skyToPixel(getSkyOrigin()));
    }
    return *_pixelOrigin;
}
...
std::shared_ptr<SkyWcs> SkyWcs::copyAtShiftedPixelOrigin(Extent2D const& shift) const {
    std::vector<double> newToOldPixelVec{-shift[0], -shift[1]};
    auto newToOldPixel = TransformPoint2ToPoint2(ast::ShiftMap(newToOldPixelVec));
    auto shiftedWcs = makeModifiedWcs(newToOldPixel, *this);
    return std::make_shared<SkyWcs>(*shiftedWcs);
}

std::shared_ptr<daf::base::PropertyList> SkyWcs::getFitsMetadata(bool precise) const {
    // Make a FrameSet that maps from GRID to SKY; GRID = PIXEL + 1
    // (ignore ACTUAL_PIXELS, if present, as it cannot be represented with FITS WCS)
    std::vector<double> gridToPixelArray = {-1.0, -1.0};
    auto const gridToPixel = ast::ShiftMap(gridToPixelArray);
    auto thisDict = getFrameDict();
    auto const pixelToIwc = thisDict->getMapping("PIXEL", "IWC");
...
std::shared_ptr<ast::FrameDict> SkyWcs::_checkFrameDict(ast::FrameDict const& frameDict) const {
    ...
    std::vector<std::string> const domainNames = {"ACTUAL_PIXELS", "PIXEL", "IWC", "SKY"};
    ...
    // The base frame must have domain "PIXEL" or "ACTUAL_PIXELS"
    auto baseDomain = frameDict.getFrame(ast::FrameSet::BASE, false)->getDomain();
    if (baseDomain != "ACTUAL_PIXELS" && baseDomain != "PIXEL") {
        throw LSST_EXCEPT(lsst::pex::exceptions::InvalidParameterError,
                          "Base frame has domain " + baseDomain + " instead of PIXEL or ACTUAL_PIXELS");
    }
...
std::shared_ptr<SkyWcs> makeFlippedWcs(SkyWcs const& wcs, bool flipLR, bool flipTB,
    ...
    return makeModifiedWcs(flipPix, wcs);
}
...
std::shared_ptr<SkyWcs> makeModifiedWcs(TransformPoint2ToPoint2 const& pixelTransform, SkyWcs const& wcs) {
    ...
    auto const pixelFrame = oldFrameDict->getFrame("PIXEL", false);
    auto const iwcFrame = oldFrameDict->getFrame("IWC", false);
    auto const skyFrame = oldFrameDict->getFrame("SKY", false);
    auto const oldPixelToIwc = oldFrameDict->getMapping("PIXEL", "IWC");
    auto const iwcToSky = oldFrameDict->getMapping("IWC", "SKY");

    auto const pixelMapping = pixelTransform.getFrameSet()->getMapping();
    auto const newPixelToIwc = pixelMapping->then(*oldPixelToIwc).simplify();

    ast::FrameDict newFrameDict(*pixelFrame, *newPixelToIwc, *iwcFrame);
...
std::shared_ptr<SkyWcs> makeTanSipWcs(Point2D const& crpix, coord::IcrsCoord const& crval,
    ...
    auto metadata = detail::makeTanSipMetadata(crpix, crval, cdMatrix, sipA, sipB);
...
std::shared_ptr<SkyWcs> makeTanSipWcs(Point2D const& crpix, coord::IcrsCoord const& crval,
    ...
    auto metadata = detail::makeTanSipMetadata(crpix, crval, cdMatrix, sipA, sipB, sipAp, sipBp);
    return std::make_shared<SkyWcs>(*metadata);
}
