...
SkyWcs::SkyWcs(daf::base::PropertyList& metadata) : SkyWcs(detail::readLsstSkyWcs(metadata)) {}
...
Angle SkyWcs::getPixelScale(Point2D const& pixel) const {
    ...
    // Compute |cross product| = area of parallelogram with sides dx,dy, in radians^2
    // This is slightly incorrect -- it's making the small-angle
    // approximation, taking the distance *through* the unit sphere
    // rather than over its surface, but the difference should be negligible for pixels of normal size
    double pixAreaRadSq = std::sqrt(square(skyDx[1] * skyDy[2] - skyDx[2] * skyDy[1]) +
                                    square(skyDx[2] * skyDy[0] - skyDx[0] * skyDy[2]) +
                                    square(skyDx[0] * skyDy[1] - skyDx[1] * skyDy[0]));
    return std::sqrt(pixAreaRadSq) * radians;
}
...
Eigen::Matrix2d SkyWcs::getCdMatrix() const {
    ...
    if (!iwcFrameSet) {
        throw LSST_EXCEPT(lsst::pex::exceptions::RuntimeError, "IWC frame not found");
    }
    auto iwcTransform = Transform<Point2Endpoint, Point2Endpoint>(*iwcFrameSet);
    return iwcTransform.getJacobian(getPixelOrigin());
}

SkyWcs SkyWcs::getTanWcs(Point2D const& pixel) const {
    // I assume the CD matrix is independent of pixel position;
    // if we find a case where this is not so, add a version of getCdMatrix that accepts pixel position
    auto const crval = tranForward(pixel);
    auto const cdMatrix = getCdMatrix();
    return SkyWcs(pixel, crval, cdMatrix);
}

SkyWcs SkyWcs::shiftedPixelOrigin(double dx, double dy) const {
    auto frameSet = getFrameSet()->copy();
    ...
    if (!frameSet->findFrame(ast::Frame(2, "Domain=GRID"))) {
        throw LSST_EXCEPT(lsst::pex::exceptions::RuntimeError, "GRID frame not found");
    }
    ...
    if (!oldGridToPixel0Map) {
        throw LSST_EXCEPT(lsst::pex::exceptions::RuntimeError, "PIXEL0 frame not found");
    }
    ...
    std::vector<double> shiftVec = {dx, dy};
