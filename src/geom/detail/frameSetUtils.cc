...
std::shared_ptr<ast::FrameSet> readFitsWcs(daf::base::PropertyList& metadata, bool strip) {
    ...
    auto const initialNames = strip ? setFromVector(channel.getCardNames()) : std::set<std::string>();
    ...
    if (strip) {
        auto const finalNames = setFromVector(channel.getCardNames());
        ...
        // TODO remove TIMESYS from this list once we update AST 2017-04-24 or newer
...
std::shared_ptr<ast::FrameSet> readLsstSkyWcs(daf::base::PropertyList& metadata, bool strip) {
    ...
    if (!frameSet->findFrame(stdSkyFrame)) {
        throw LSST_EXCEPT(lsst::pex::exceptions::RuntimeError, "Could not find a SkyFrame");
    }
    ...
    // What we do is look for a frame named "GRID"; if that is found then we use it (since that follows
    // the standard AST domain convention). If that is not found then we check if the base frame
    // is of type "Frame" and has a blank Domain, and if so, use that.

    // Look for a Frame with Domain "GRID"; if found assume it is the standard FITS pixel frame
    auto gridIndex = ast::FrameSet::NOFRAME;
    if (frameSet->findFrame(ast::Frame(2, "Domain=GRID"))) {
        gridIndex = frameSet->getCurrent();
    }

    // If no GRID Frame was found, but the original base has a blank domain
    // then assume that is the standard FITS pixel frame. Set its domain to GRID,
    // and set some other potentially useful attributes.
    if (gridIndex == ast::FrameSet::NOFRAME) {
        ...
        if (baseClassName != "Frame") {
            std::ostringstream os;
            os << "Could not find a pixel frame; no frame with domain GRID and the base frame is of type "
               << baseClassName << "instead of Frame";
            throw LSST_EXCEPT(lsst::pex::exceptions::RuntimeError, os.str());
        }
        auto const baseDomain = frameSet->getDomain();
        if (baseDomain != "") {
            std::ostringstream os;
            os << "Could not find a pixel frame; no frame with domain GRID and the base frame has domain \""
               << baseDomain << "\" instead of blank";
            throw LSST_EXCEPT(lsst::pex::exceptions::RuntimeError, os.str());
        }
        frameSet->setDomain("GRID");
        frameSet->setTitle("FITS pixel coordinates - first pixel at (1,1).");
        frameSet->setLabel(1, "Grid coordinate 1");
        frameSet->setLabel(2, "Grid coordinate 2");
        frameSet->setUnit(1, "Pixel");
        frameSet->setUnit(2, "Pixel");
        frameSet->setSymbol(1, "g1");
        frameSet->setSymbol(2, "g2");
    ...
    auto pixelFrame = ast::Frame(2,
                                 "Domain=PIXEL0,"
                                 "Title=Pixel coordinates within parent image,"
                                 "Label(1)=Pixel coordinate 1,"
                                 "Label(2)=Pixel coordinate 2,"
                                 "Unit(1)=Pixel, Unit(2)=Pixel,"
                                 "Symbol(1)=p1, Symbol(2)=p2");
