...
public:
	try_fits_read_ramImage(char **ramFile, size_t *ramFileLen, ImageT& img,
						lsst::daf::base::PropertySet::Ptr metadata,
						int hdu,
						lsst::afw::image::BBox const& bbox
						) : _ramFile(ramFile), _ramFileLen(ramFileLen), _img(img), _metadata(metadata), _hdu(hdu), _bbox(bbox) { }
	
	void operator()(ImageT) {           // read directly into the desired type if the file's the same type
		try {
			lsst::afw::image::fits_read_ramImage(_ramFile, _ramFileLen, _img, _metadata, _hdu, _bbox);
			throw ExceptionT();         // signal that we've succeeded
		} catch(lsst::afw::image::FitsWrongTypeException const&) {
			// ah well.  We'll try another image type
		}
	...
	template<typename U> void operator()(U) { // read and convert into the desired type
		try {
			U img;
			lsst::afw::image::fits_read_ramImage(_ramFile, _ramFileLen, img, _metadata, _hdu, _bbox);
...
template<typename fits_img_types, typename ImageT>
bool fits_read_ramImage(char **ramFile, size_t *ramFileLen, ImageT& img,
					 lsst::daf::base::PropertySet::Ptr metadata = lsst::daf::base::PropertySet::Ptr(),
					 int hdu=0,
					 BBox const& bbox=BBox()
					 ) {
	try {
		boost::mpl::for_each<fits_img_types>(try_fits_read_ramImage<ImageT, found_type>(ramFile, ramFileLen, img, metadata, hdu, bbox));
	} catch (found_type &) {
		return true;                    // success
	}
