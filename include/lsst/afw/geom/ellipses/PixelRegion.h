...
/**
 *  @brief A footprint-like object that pixelizes an Ellipse
 *
 *  PixelRegion behaves like a container of Spans, but these are actually computed
 *  on-the-fly.
 */
class PixelRegion {
public:
    ...
    /// Return an iterator to the first span in the region
    Iterator begin() const;

    /// Return an iterator to one past the last span in the region
    Iterator end() const;
#endif

    /// Return the bounding box of the region (precomputed at construction)
    Box2I const & getBBox() const { return _bbox; }

    /// Return a Span containing the x-extent of the pixelized ellipse at the for the given y value
