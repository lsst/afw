...
*/
class SkyWcs : public table::io::PersistableFacade<SkyWcs>, public table::io::Persistable {
public:
...
private:
    ...
    mutable std::shared_ptr<Point2D> _pixelOrigin;  // cached value
    mutable Angle _pixelScaleAtOrigin;              // cached value
...
/**
 * Create a new SkyWcs that acts like pixelTransform.then(wcs)
 *
 * In order to preserve the meaning of the standard SkyWcs frames, pixelTransform is inserted
 * between the PIXELS and IWC frames:
 * newGridToIwc = gridToPixel0 -> pixelTransform -> pixel0ToGrid -> oldGridToIwc
 */
