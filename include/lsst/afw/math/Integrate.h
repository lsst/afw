...
// If defined, then count the number of function evaluations
  ...
  template <class T> struct IntRegion 
  {
...
namespace details {            
  ...
  template <class T> inline T MinRep() 
  { return std::numeric_limits<T>::min(); }

/*
  template <> inline double Epsilon<double>()
  { return 2.2204460492503131e-16; }
  template <> inline double MinRep<double>()
  { return 2.2250738585072014e-308; }
*/

#ifdef EXTRA_PREC_H
  template <> inline Quad Epsilon<Quad>()
  { return 3.08148791094e-33; }
  template <> inline Quad MinRep<Quad>()
  { return 2.2250738585072014e-308; }
#endif

  template <class T> inline T rescale_error (
      T err, const T& resabs, const T& resasc)
  {
    if (resasc != 0. && err != 0.) {
  ...
  template <class UF> inline bool IntGKPNA(
    ...
    // the two schemes, so I haven't bothered to calculate any more.
  {
	...
	if (calcabsasc) 
	  resabs += gkp_wa<T>(level)[k] * (fabs(fv1[k]) + fabs(fv2[k]));
      }
  ...
  template <class UF> inline void IntGKP (
      const UF& func, IntRegion<typename UF::result_type>& reg,
      const typename UF::result_type epsabs,
      const typename UF::result_type epsrel,
      std::map<typename UF::result_type,
  ...
  template <class UF> struct AuxFunc1 : // f(1/x-1) for int(a..infinity)
  ...
  template <class UF> AuxFunc1<UF> inline Aux1(UF uf) 
  ...
  template <class UF> AuxFunc2<UF> inline Aux2(UF uf) 
  ...
  template <class T> struct ConstantReg1 : 
  ...
  template <class T> struct ConstantReg2 :
  ...
  template <class BF, class YREG> class Int2DAuxType : 
  ...
  template <class TF, class YREG, class ZREG> class Int3DAuxType : 
  ...
  template <class UF> inline typename UF::result_type int1d(
      const UF& func, 
  ...
  template <class BF, class YREG> inline typename BF::result_type int2d(
      const BF& func,
  ...
  template <class BF> inline typename BF::result_type int2d(
      const BF& func,
      IntRegion<typename BF::result_type>& reg,
  ...
  {
    using namespace details;
    return int2d(func,reg,ConstantReg1<typename BF::result_type>(yreg),
	abserr,relerr); 
  }

  template <class TF> inline typename TF::result_type int3d(
      const TF& func,
      IntRegion<typename TF::result_type>& reg,
 ...
 *
 * @note This simply wraps the int1d function above
 *
 */
...
public:
    ...
    typename BinaryFunctionT::result_type operator() (typename BinaryFunctionT::second_argument_type const y) const {
